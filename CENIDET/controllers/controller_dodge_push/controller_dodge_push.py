"""dodge_push_controller controller."""
...
#####Funcion Vrot
    ...
    with open("vrot_data_dodge_push.csv", newline="") as f:
        ...
        for row in reader:
            ...
            H = int(row["H"])
            modelo[(dir_val, prox_val, H)].append(vrot_val)
...
####### Main loop:
    ...
    while robot.step(TIMESTEP) != -1:
        ...
        """  
        if ((dir_val == -1 and prox_val >= 5) or (dir_val == 1 and prox_val >= 5) 
        or (dir_val == -2 and prox_val >= 5) or (dir_val == 2 and prox_val >= 5)):
            H = 1  
        else:
            H = 0 
            
        if (dir_val, prox_val, H) in tabla:
            media, sigma = tabla[(dir_val, prox_val, H)]
