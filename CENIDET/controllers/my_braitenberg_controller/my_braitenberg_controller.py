"""braitenberg_my_controller controller."""
...
#####Funcion Vrot
    ...
    def compute_vrot(dir_val, prox_val):
        
        max_rotation = 5
        
        dir_normalized = dir_val / 3
        prox_normalized = prox_val / 10
        
        vrot = max_rotation * dir_normalized * prox_normalized
        vrot = math.ceil(vrot) 
        
        return vrot  
        
        
# Guardar lista de vrot por cada (dir_val, prox_val)
...
####### Main loop:
    ...
    while robot.step(TIMESTEP) != -1:
        ...
        esta en el intervalo [-4, -3, -2, -1, 0, 1, 2, 3, 4]
        
        """  
        vrot = compute_vrot(dir_val, prox_val)
        vtrans = 3
        ...
        observaciones[(dir_val, prox_val)].append(vrot)
...
########Impresiones         
    ...
    with open("vrot_data.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Dir", "Prox", "Vrot"])
        for (dir_val, prox_val), vrot_list in observaciones.items():
            for vrot in vrot_list:
                writer.writerow([dir_val, prox_val, vrot])
